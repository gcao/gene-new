...
proc init*() =
  GeneTranslators["if"] = proc(v: Value): Value =
    result = Value(
      kind: VkExIf,
    )
    result.ex_if_cond = translate(v.gene_props[COND_KEY])
    result.ex_if_then = translate(v.gene_props[THEN_KEY])
    if v.gene_props.has_key(ELIF_KEY):
      var elifs = v.gene_props[ELIF_KEY]
      var i = 0
      while i < elifs.vec.len:
        ...
        i += 2
    result.ex_if_else = translate(v.gene_props[ELSE_KEY])
